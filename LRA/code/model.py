...
class Transformer(nn.Module):
    ...
    def forward(self, X, mask):
        if self.config['attn_type'].startswith('dct'):
            shortX, X = self.mha(self.norm1(X), mask)
            X = self.dropout1(X) + shortX
        else:
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        if self.config['attn_type'].startswith('dct'):
            smconfig = config.copy()
            smconfig['attn_type'] = 'softmax'
            for idx in range(self.num_layers):
                if idx in config['layers']:
                    setattr(self, f"transformer_{idx}", Transformer(config))
                else:
                    setattr(self, f"transformer_{idx}", Transformer(smconfig))
        else:
            for idx in range(self.num_layers):
                setattr(self, f"transformer_{idx}", Transformer(config))
