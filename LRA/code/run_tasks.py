...
parser.add_argument("--log_dir", type = str, help = "log_dir", dest = "log_dir", required = False)
parser.add_argument("--layers", type = str, help = "layers", dest = "layers", required = False)
...
model_config['layers'] = [int(s) for s in layers.split(',')]
...
log_f_path = os.path.join(checkpoint_dir, f"{task}2k_{attn_type}_{layers}_output.log")
log_f = open(log_f_path, "a+")

train_writer = SummaryWriter(os.path.join(log_dir, f"{task}_{attn_type}_{layers}_train"))
dev_writer = SummaryWriter(os.path.join(log_dir, f"{task}_{attn_type}_{layers}_dev"))
test_writer = SummaryWriter(os.path.join(log_dir, f"{task}_{attn_type}_{layers}_test"))
...
accumu_steps = max(training_config["batch_size"] // len(device_ids) // gpu_memory_config, 1)
    ...
    try:
        ...
        for train_step_idx in range(training_config["num_train_steps"]):
            ...
            if (train_step_idx + 1) % training_config["eval_frequency"] == 0:
                train_summary_round = print_summary(summary["train"], False, train_step_idx)
                train_writer.add_scalar('time_per_step', train_summary_round['t'], train_step_idx)
                train_writer.add_scalar('loss', train_summary_round['loss'], train_step_idx)
                train_writer.add_scalar('accuracy', train_summary_round['accu'], train_step_idx)
                train_writer.add_scalar('best_accracy', train_summary_round['best_accu'], train_step_idx)
                model.eval()
                for dev_step_idx in range(training_config["num_eval_steps"]):
                    outputs = step("dev", dev_step_idx)
                dev_summary_round = print_summary(summary["dev"], True, train_step_idx)
                dev_writer.add_scalar('time_per_step', dev_summary_round['t'], train_step_idx)
                dev_writer.add_scalar('loss', dev_summary_round['loss'], train_step_idx)
                dev_writer.add_scalar('accuracy', dev_summary_round['accu'], train_step_idx)
                dev_writer.add_scalar('best_accracy', dev_summary_round['best_accu'], train_step_idx)
                model.train()
...
try:
    ...
    test_summary_round = print_summary(summary["test"], False, training_config["num_train_steps"])
    test_writer.add_scalar('time_per_step', test_summary_round['t'], training_config["num_train_steps"])
    test_writer.add_scalar('loss', test_summary_round['loss'], training_config["num_train_steps"])
    test_writer.add_scalar('accuracy', test_summary_round['accu'], training_config["num_train_steps"])
    test_writer.add_scalar('best_accracy', test_summary_round['best_accu'], training_config["num_train_steps"])
