...
class TransformerEncoderBase(FairseqEncoder):
    ...
    def forward_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx >= min(ls_layer):
                has_pads = False
                encoder_padding_mask = torch.zeros(encoder_padding_mask.shape).to(encoder_padding_mask.device)

            if self.cfg.global_residual and idx in ls_layer:
                global_residual.append(x)

            if idx in ls_layer:
                if self.cfg.arch.startswith('roberta'):
                    # seperate CLS in roberta
                    cls = x[0, :, :].unsqueeze(0)
                    x = self.dc_transform(x[1:, :, :])
                    x = torch.cat([cls, x], dim=0)
                elif self.cfg.arch.startswith('bart'):
                    x = self.dc_transform(x)

            lr = layer(
                x, idx, global_residual,
    ...
    def dc_transform(self, x):
        ...
        if self.cfg.dct_lowpass:
            x_dct = x_dct[:math.ceil(T*self.cfg.dct_percentage), :, :]
